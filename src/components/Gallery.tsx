                ...
                {categories.map((category) => (
                    <div key={category.name} className="gallery-card" onClick={() => openGallery(category)}>
                        <img 
                            ...
                            alt={category.name} 
                            onError={(e) => {
                                console.error(`Failed to load image: /images/${category.images[0]}`);
                                e.currentTarget.style.display = 'none';
                            }}
                            onLoad={() => console.log(`Successfully loaded image: /images/${category.images[0]}`)}
                        />
                        <h3>{category.name}</h3>
                    </div>
                ))}
                    ...
                    <div className="modal-content">
                        ...
                        <div className="image-viewer">
                            ...
                            <img 
                                ...
                                onError={(e) => {
                                    console.error(`Failed to load modal image: /images/${selectedCategory.images[currentIndex]}`);
                                }}
                                onLoad={() => console.log(`Successfully loaded modal image: /images/${selectedCategory.images[currentIndex]}`)}
                            />
